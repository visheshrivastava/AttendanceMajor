...
public class DBAdapter extends SQLiteOpenHelper {
	...
	// Database Version
	private static final int DATABASE_VERSION = 11; // Increment this
	...
	public ArrayList<AttendanceBean> getAttendanceBySessionID(AttendanceSessionBean attendanceSessionBean) {
		...
		String query = "SELECT * FROM " + ATTENDANCE_TABLE + 
					  " WHERE " + KEY_SESSION_ID + " = " + attendanceSessionBean.getAttendance_session_id();
		...
		if (cursor.moveToFirst()) {
			do {
				...
				attendanceBean.setAttendance_student_id(cursor.getInt(cursor.getColumnIndex(KEY_ATTENDANCE_STUDENT_ID)));
	...
	public ArrayList<AttendanceBean> getAttendanceBySessionIDAndSession(long sessionId, String session) {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				Log.d("DBAdapter", "Retrieved attendance record - SessionID: " + 
